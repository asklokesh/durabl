...
These are on the roadmap from Phase 0 / build plan; **there is no adapter code** in
this repo yet. Do not expect drop-in LangGraph or Pydantic AI wiring today.

| Integration | Intent | Status |
|-------------|--------|--------|
| **LangGraph** | Map graph nodes/checkpoints to durabl `JournalEntry` + idem keys; optional checkpointer backend | **Planned** — Python SDK track |
...
- **Orchestration stays in the framework** (graph edges, handoffs, tools).
- **durabl owns** per-step outputs, idempotency keys, effect dedup, export JSONL,
  replay UI, logical fork, and HITL journal records.
- Each framework adapter should: (a) assign stable step names per logical unit of
  work, (b) route side effects through the same `fireEffect` contract (or language
  equivalent), (c) avoid re-invoking models/tools when the journal already has
  `(runId, seq)`.

Until then, the **authoritative integration example** is the TypeScript Restate
workflow in `src/workflow.ts` plus HITL in `src/hitl-workflow.ts`.
