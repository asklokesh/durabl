...
// ── Connection banner (API health) ─────────────────────────
function updateConnectionBanner() {
  const pill = $("#sourcePill");
  const offline = !state.live;
  pill.classList.toggle("offline", offline);
  pill.title = offline ? state.hitlSubmitDisabledReason : "Live journal with Restate ingress";
}

function hitlCanSubmit() {
  return state.hitlSubmitEnabled && state.live;
}

function syncHitlSubmitTooltip() {
  const wrap = $("#hitlSubmitWrap");
  const btn = $("#hitlSubmitBtn");
  const canSubmit = hitlCanSubmit();
  btn.disabled = !canSubmit;
  wrap.title = canSubmit ? "" : state.hitlSubmitDisabledReason;
}
...
async function loadSource() {
  ...
  state.hitlSubmitDisabledReason =
    h.hitlSubmitDisabledReason || HITL_SUBMIT_OFFLINE_ERROR;
...
async function submitHitlInput(ev) {
  ...
  const btn = $("#hitlSubmitBtn");
  if (!state.selected || !decision || !hitlCanSubmit()) return;
  btn.disabled = true;
  msg.hidden = false;
...
function updateHitlBanner() {
  ...
  $("#hitlDecision").disabled = !canSubmit;
  syncHitlSubmitTooltip();
}
